...
export async function fetchLast10Users() {
  try {
    const users = await sql<User>`SELECT * FROM users ORDER BY created_at DESC LIMIT 10`;
    return users.rows;
  } catch (error) {
    console.error('Failed to fetch last 10 users:', error);
    throw new Error('Failed to fetch last 10 users.');
  }
}
...
export async function fetchUserById(id: string): Promise<UserForm | null> {
  ...
  try {
    ...
    return data.rows[0]; // Return the first user object
  } catch (error) {
